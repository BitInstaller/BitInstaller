...
class InstallationEngine:
    ...
    def splitWimfile(self):
        self.copyISOCommand = subprocess.Popen(
            [
                "wimlib-imagex",
                "split",
                self.mountedISO + "/sources/install.wim",
                "/Volumes/WIN10/sources/install.swm",
                "3800",
            ],
    ...
    def copyISOFiles(self):
        os.system(
            f"rsync -vha --exclude=sources/install.wim {self.mountedISO}/* /Volumes/WIN10"
        )
        # self.copyISOCommand = subprocess.Popen(
    ...
    def mountSelectedISO(self):
        ...
        parts = (
            self.mountOutput.split()
        )  # Split the string using default whitespace delimiter
    ...
    def formatDrive(self):
        ...
        try:
            self.formatDiskCommand = subprocess.Popen(
                ["diskutil", "eraseDisk", "MS-DOS", "WIN10", "GPT", self.selectedDrive],
                stdout=subprocess.PIPE,
            )
            ...
            try:
                ...
                (
                    self.formatOutput,
                    self.formatError,
                ) = self.formatDiskCommand.communicate()
            except Exception:
                return False
